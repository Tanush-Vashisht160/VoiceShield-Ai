...
class VoiceSecurityFirewall:
    ...
    def analyze_call(
        ...
        # ------------------------------------------------------------
        # 0. AVAILABILITY FLAGS INITIALIZATION
        # ------------------------------------------------------------
        ...
        context_risk_score_value = 0.0
        context_availability = 0.0

        # Prefer the analysis transcript when one is available.
        # Otherwise use the original transcript.
        context_text = (
            analysis_transcript
            if analysis_transcript
            else transcript
        )

        # ------------------------------------------------------------
        # CASE 1: Explicit context risk score was supplied
        # ------------------------------------------------------------

        if context_risk_score is not None:

            context_risk_score_value = self._validate_score(
                context_risk_score,
                "context_risk_score",
            )
        ...
        # ------------------------------------------------------------
        # CASE 2: Transcript is available
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # 4. SPEAKER VERIFICATION & MISMATCH SCORE
        # ------------------------------------------------------------

        speaker_result = None
        speaker_mismatch_score_value = 0.0
        speaker_availability = 0.0

        # ------------------------------------------------------------
        # CASE 1: Explicit speaker mismatch score was supplied
        # ------------------------------------------------------------

        if speaker_mismatch_score is not None:

            speaker_mismatch_score_value = self._validate_score(
                speaker_mismatch_score,
                "speaker_mismatch_score",
            )
        ...
        # ------------------------------------------------------------
        # CASE 2: Reference audio is available
        # ------------------------------------------------------------
            ...
            similarity = self._validate_score(
            ...
            #
            # similarity = 1.0 -> perfect match -> mismatch 0.0
            # similarity = 0.0 -> no match -> mismatch 1.0
            speaker_mismatch_score_value = (
                1.0 - similarity
            )
        ...
        # ------------------------------------------------------------
        # CASE 3: No speaker evidence available
        # ------------------------------------------------------------
        ...
        risk = self.risk_engine.calculate(
            fake_score=fake_score,
            speaker_mismatch_score=speaker_mismatch_score_value,
            context_risk_score=context_risk_score_value,
        ...
        return {
            ...
            "transcription": {
                ...
                "analysis_transcript": analysis_transcript or "",
            },
            "risk_inputs": {
                "fake_score": fake_score,
                "speaker_mismatch_score": speaker_mismatch_score_value,
                "context_risk_score": context_risk_score_value,
                "availability": {
                    "fake": fake_availability,
                    "speaker": speaker_availability,
